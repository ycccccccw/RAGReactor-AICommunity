...
返回方式：SSE 流式输出
网络：显式清除 HTTP_PROXY/HTTPS_PROXY/ALL_PROXY，HTTP 客户端禁用环境代理
```
...
| Recall@3 | — | **1.0000** | 不适用 |
| MRR | — | **1.0000** | 不适用 |
| Faithfulness | 0.9379 | 0.8827 | 1.0000 |
| Answer Relevancy | 0.6315 | **0.9472** | 0.0000 |
| Context Precision | 0.6667 | **1.0000** | 0.0000 |
| Context Recall | 0.9167 | **1.0000** | 0.7500 |
| Factual Correctness | 0.6036 | 0.4950 | 0.7938 |
| 正确拒答率 | — | 不适用 | **100%（8/8）** |
| 明显幻觉率 | — | — | **0%（0/8）** |
...
本轮 RAGAS 有效评分数：Faithfulness 17/24、Answer Relevancy 24/24、Context Precision 24/24、Context Recall 24/24、Factual Correctness 22/24；缺失项不按 0 填充。
...
|---|---:|---:|---:|---:|---:|
| 查询向量生成 | 237.92ms | 218.66ms | 325.60ms | 558.28ms | 8.64% |
| 向量召回与排序 | **0.58ms** | 0.56ms | 0.66ms | 0.69ms | 0.02% |
| qwen3-rerank | 189.51ms | 179.29ms | 300.02ms | 311.53ms | 6.88% |
| Prompt 构建 | **0.009ms** | 0.009ms | 0.011ms | 0.016ms | <0.01% |
| qwen-plus 首 Token（仅生成阶段） | **538.04ms** | 516.84ms | 667.38ms | 715.19ms | — |
| **端到端首字时延** | **966.05ms** | **926.41ms** | **1106.08ms** | **1396.19ms** | — |
| qwen-plus 完整返回 | **2326.65ms** | 1869.53ms | 5438.96ms | 6022.46ms | **84.46%** |
| **端到端完整返回** | **2754.66ms** | **2276.39ms** | **5864.80ms** | **6463.44ms** | 100% |

3 份知识文档的离线 Embedding 构建总耗时为 550.30ms。该耗时发生在索引构建阶段，不进入在线请求总时延。

首 Token 有两个口径：“qwen-plus 首 Token”只统计请求生成模型到收到第一个有效 Token；“端到端首字时延”还包含查询 Embedding、向量召回、Rerank 和 Prompt 构建。用户真实感受到的是后者，平均 966.05ms、P95 1.11 秒。相比等待平均 2.75 秒的完整回答，流式输出把平均可感知等待缩短约 65%。

### 13.4 瓶颈分析

第一瓶颈是大模型生成。qwen-plus 占平均完整返回时延的 84.46%，并贡献几乎全部长尾：LLM P99 为 6.02 秒，而端到端 P99 为 6.46 秒。向量检索和 Prompt 拼接合计不到 1ms，即使继续优化 C++ 字符串或余弦计算，对用户体验几乎没有可见改善。

第二层开销是查询 Embedding 和 Rerank，合计平均 427.43ms，占 15.52%。Rerank 把前面的 MRR 从 0.883 提高到 0.985，因此默认保留是合理的；若接口需要极低延迟，可以在“向量首名分数明显领先”时跳过 Rerank，但必须重新测量质量下降。

建议按收益排序优化：

1. 保持 SSE 流式输出并监控端到端 TTFT。本轮首字 P95 为 1.11 秒，而完整返回 P95 为 5.86 秒，流式输出收益明确。
